...
const getWebinarByEventDateIdAndWebinarId = async (webinarId:string, eventDateId: string): Promise<IWebinar> => {
    return await axiosInterceptorInstance.get(`${process.env.NEXT_PUBLIC_BASE_API_PATH_LOCAL}/eventdates/${eventDateId}?populate[0]=webinar.coverImage`).then((res) => {
        return res.data.data;
    })
}

const getWebinarById = async (webinarId:string) => {
    return axiosInterceptorInstance.get(`${process.env.NEXT_PUBLIC_BASE_API_PATH_LOCAL}/webinars/${webinarId}?populate[0]=author.image&populate[1]=event_dates&populate[2]=coverImage`).then((res) => {
